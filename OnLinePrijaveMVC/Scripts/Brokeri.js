    ...
    $(function () {
        $("#form1").validate({
            ...
            messages: {
                ...
                "DokazIzClankaPet": {
                    required: "Dokaz o odgovarajućem stupnju obrazovanja (članak 5. (2) i (3)) je obavezan podatak",
                    maxFilesLenValid: "Maximalna dozvoljena veličina datoteka za dokaz o članku 5. je 5 MB"
                },
                "PotvrdaRadnogIskustva": {
                    required: "Potvrda o najmanje šest mjeseci odgovarajućeg radnog iskustva je obavezan podatak",
                    maxFilesLenValid: "Maximalna dozvoljena veličina datoteka za potvrdu o najmanje šest mjeseci odgovarajućeg radnog iskustva je 5 MB"
                },
                "DokazOPolozenimIspitima": {
...
function blockUIForDownload() {
    fileDownloadCheckTimer = window.setInterval(function () {
        if ($.cookie('onLinePrijaveBrokeriDownloadPDF')) {
            finishDownload();
        }
    }, 1000);
}
...
function finishDownload() {
    ...
    setCookie('onLinePrijaveBrokeriDownloadPDF', 'null', -1);

    $('#generiranjePotvrde').modal('hide');

    // clear input files
    $("#PotvrdaRadnogIskustva").val(null);
...
function UploadFiles(registrationDate) {
    ...
    var potvrdaRadnogIskustva = document.getElementById('PotvrdaRadnogIskustva').files;
    ...
    for (var i = 0, l = potvrdaRadnogIskustva.length; i < l; i++) {
        totalByteLength += potvrdaRadnogIskustva[i].size;
    }
    ...
    $('#filesToUpload').val(dokazIzClankaPet.length + dokazOPolozenimIspitima.length + dokazOPodmirenojNaknadi.length + potvrdaRadnogIskustva.length);
    ...
    for (var i = 0; i < potvrdaRadnogIskustva.length; i++) {
        UploadFile(potvrdaRadnogIskustva[i], 'PotvrdaRadnogIskustva', i + 1, registrationDate);
    }
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                if (data.status == true) {
                    ...
                    JL("Brokeri_UploadFileChunk").fatal("failed to upload file part no: " + CurrentPart);
...
function DownloadPDF() {
    try {
        JL("Brokeri_DownloadPDF").info("Počinjem download PDF potvrde.");
        ...
        window.location = serverRoot + '/Brokeri/DownloadPDF?DownloadToken=' + token;

        JL("Brokeri_DownloadPDF").info("Gotov download PDF potvrde.");
    } catch (e) {
        JL("Brokeri_DownloadPDF").fatalException("Greška kod download potvrde", e);
    }
}

function SaveDataToDatabase() {
    JL("Brokeri_SaveDataToDatabase").info("Snimam podatke u bazu.");
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                if (data.status == true) {
                    ...
                    JL("Brokeri_SaveDataToDatabase").fatal(
                        {
                            "msg": "Greška",
                            "poruka": data.message
                        });
            ...
            error: function (xhr, textStatus, thrownError) {
                JL("Brokeri_SaveDataToDatabase").fatal("Snimanje podataka nije uspjelo");
                $('#pohranaPodataka').modal('hide');
                alert('Snimanje podataka nije uspjelo');
            }
        });
    ...
    JL("Brokeri_SaveDataToDatabase").info("Gotovo snimanje podataka u bazu.");
}
...
function SaveFormData() {
    ...
    var formData = new FormData();
    formData.append('VrstaPrijaveBroker', $("input[name='VrstaPrijaveBroker']:checked").val());
    ...
    jQuery(function ($) {
        $.ajax({
            ...
            error: function (xhr, textStatus, thrownError) {
                ...
                console.log("xhr.status: " + xhr.status);
                console.log("textStatus: " + textStatus);
                JL("Brokeri_SaveFormData").fatal("Snimanje forme nije uspjelo");
